...
class Class
{
   ...
   static double Funkcja_przystosowania(double pa, double pb, double pc, double[] X, double[] Y)
   {
    ...
    static List<(string,double)> ocen_osobnika(List<(string,double,double,double)> Dekodowanie, double[] X, double[] Y)
    {
        ...
        {
            Console.WriteLine("{0}  {1}\t", krotka.Item1, krotka.Item2);
        }
        return osobnicy;
    }
    static List<(string,double)> Turniej (List<(string,double)> Pula, int liczba_osobnikow)
    {
        List<(string, double)> osobnik_po_turnieju = new List<(string, double)>();
        Random rnd = new Random();
        for (int i=0; i< liczba_osobnikow-1; i++)
        {
            ...
            {
                osobnik_po_turnieju.Add(Pula[osobnik1]);
            }
            else if (Pula[osobnik1].Item2 < Pula[osobnik2].Item2)
            {
                osobnik_po_turnieju.Add(Pula[osobnik2]);
            }
        }
        foreach( var i in osobnik_po_turnieju)
        {
            Console.WriteLine(i);
        }
        return osobnik_po_turnieju;
    }
    static List<string> Krzyżowanie(List<(string,double)> Nowa_Pula, int liczba_chromosomow)
    {
        List<string> Pula1 = new List<string>();
        List<(string, double)> Pula2 = new List<(string, double)>();
        List<(string, double)> Pula3 = new List<(string, double)>();
        List<(string, double)> Pula4 = new List<(string, double)>();
        Random rnd = new Random();
        int[] indeksy = { 0, 2, 8, 10 };
        foreach(var i in indeksy)
        {
            int b = liczba_chromosomow;
            Console.WriteLine(b);
            string rodzic1 = Nowa_Pula[i].Item1;
            string rodzic2 = Nowa_Pula[i + 1].Item1;
            ...
            Pula1.Add(krzyzowanie1);
            Pula1.Add(krzyzowanie2);
        }
        
        foreach (var i in Pula1)
        {
            Console.WriteLine(i);
        }
        return Pula1;
    ...
    static void Main(string[] args)
    {
        ...
        {
            List<(string, double)> turniej = Turniej(ocena_osobnika, liczba_osobnikow);
            List<string> Pula_krzyzowana = Krzyżowanie(turniej, liczba_chromosomow);
        }
